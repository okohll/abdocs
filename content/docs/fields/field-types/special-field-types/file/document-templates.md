...
| $view.htmlTablePivot("table name", "child view name")        | Create a [pivot table](https://en.wikipedia.org/wiki/Pivot_table) output. The first column in the view will be the heading (portion size in the example below). The second will be the 'vertical' category (nutrition in the example) and all other fields will be values                                                                              |

Pivot table example: source view
![pivot table source data](/pivot-source.png)

Pivot table example: output
